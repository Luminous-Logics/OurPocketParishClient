...
export const defaultValues: CreateChurchFormType = {
  ...
  timezone: { label: "America/Chicago", value: "America/Chicago" }, // Default value
  subscription_plan: { label: "Basic", value: "basic" }, // Default value, aligned with backend enum
...
// ============================================
// CHURCH MANAGEMENT SPECIFIC TYPES
// ============================================
...
export const createChurchSchema = z.object({
  ...
  const anyAdminFieldProvided = adminFields.some(field => field !== undefined && field !== null && field !== "");

  if (anyAdminFieldProvided) {
    if (!data.admin_email) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Admin Email is required when other admin fields are provided",
        path: ["admin_email"],
      });
    }
    if (!data.admin_password) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Admin Password is required when other admin fields are provided",
        path: ["admin_password"],
      });
    }
    if (!data.admin_first_name) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Admin First Name is required when other admin fields are provided",
    ...
    if (!data.admin_last_name) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Admin Last Name is required when other admin fields are provided",
    ...
    if (!data.admin_phone) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Admin Phone is required when other admin fields are provided",
        path: ["admin_phone"],
      });
    }
    if (!data.admin_role) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Admin Role is required when other admin fields are provided",
        path: ["admin_role"],
      });
    }
  }
